...
from config import dockercli, client, USERID, USERCONTAINERS, USERNAME, db
from db_cfg import UsersContainers, Container
import requests
...
def showContainerCreation():
    data = requests.get('http://localhost:8000/images/json')
    images = json.loads(data.text)
    return render_template('containers/creator.html', images=images['images'])
